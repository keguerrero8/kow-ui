class PharmacistService {
    ...
    async updatePharmacist (pharmacist_id, obj) {
        const response = await fetch(`${process.env.NEXT_PUBLIC_NEXT_API_URL}/api/pharmacists/${pharmacist_id}`, {
            method: "PUT",
            headers: { 
                "Content-Type": "application/json", 
            },
    ...
    async deletePharmacist (pharmacist_id) {
        ...
        const isDeleted = await fetch(`${process.env.NEXT_PUBLIC_NEXT_API_URL}/api/pharmacists/${pharmacist_id}`, { 
            method: "DELETE",
        })
        .then(r => {
            if (r.ok) return true
